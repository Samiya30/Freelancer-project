...
export default function ProjectsPage() {
  ...
  const handleAddProject = async () => {
    ...
    const budget = Number(newProject.budget);
    ...
    const progress = Math.min(
      100,
      Math.max(0, Number(newProject.progress) || 0)
    );
    ...
    try {
      ...
      const response = await createProject({
        ...
        clientEmail: newProject.clientEmail.trim(),
        description: newProject.description.trim(),
        status: newProject.status,
        startDate: toIsoDate(newProject.startDate),
        dueDate: toIsoDate(newProject.dueDate),
        budget,
        progress,
      });
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      showToast(`${project.name} moved to ${status}.`, "success");
    } catch (error) {
      setProjects((current) =>
        current.map((item) =>
          item.id === project.id ? previous : item
        )
      );
  ...
  const handleProgressChange = async (
    ...
    const updatedStatus: ProjectStatus =
      progress === 100
        ? "Completed"
        : progress > 0 && project.status === "Planning"
        ? "In Progress"
        : project.status;
                      ...
                      <div className="flex items-center gap-5">
                        ...
                        <div className="relative shrink-0">
                          <button
                            onClick={() =>
                              setMenuId(
                                menuId === project.id ? null : project.id
                              )
                            }
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-violet-100">
                    Connect projects with tasks, time tracking, invoices and
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm">
            <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-500">
                This project will be permanently removed from your
                workspace.
              </p>
