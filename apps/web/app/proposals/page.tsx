...
function getStatusStyle(status: ProposalStatus) {
  switch (status) {
    case "Accepted":
...
function getNextProposalNumber(proposals: ApiProposal[]) {
  let highest = 0;

  for (const proposal of proposals) {
    const match = proposal.number.match(/(\d+)$/);
...
export default function ProposalsPage() {
  ...
  /*
   * Load proposals, clients and projects from the backend.
   *
   * There are intentionally no hardcoded clients/projects here.
   */
  useEffect(() => {
    let cancelled = false;
    ...
    async function loadData() {
      ...
      try {
        const [proposalResponse, clientResponse, projectResponse] =
          await Promise.all([
            getProposals(),
            getClients(),
            getProjects(),
  ...
  const filteredProposals = useMemo(() => {
    ...
    return proposals.filter((proposal) => {
      const matchesSearch =
        ...
        proposal.clientEmail.toLowerCase().includes(query) ||
  ...
  const totalValue = proposals.reduce(
    (sum, proposal) => sum + proposal.amount,
    0
  );
  ...
  const responseCount = acceptedCount + rejectedCount;

  const acceptanceRate =
    responseCount > 0
      ? Math.round((acceptedCount / responseCount) * 100)
      : 0;

  const selectedClient = clients.find(
    (client) => String(client.id) === newProposal.clientId
  );

  const selectedProject = projects.find(
    (project) => String(project.id) === newProposal.projectId
  );

  /*
   * Create proposal through the API.
   */
  const handleAddProposal = async () => {
    ...
    const amount = Number(newProposal.amount);
    ...
    try {
      const response = await createProposal({
        number: getNextProposalNumber(proposals),
        title: newProposal.title.trim(),

        // Keep the denormalized display fields in sync with the
        // actual selected Client record.
        client: selectedClient.name,
        clientEmail: selectedClient.email,

        // Proposal schema currently stores project as text.
        // We therefore use the real Project record's name.
      ...
      if (!response.data) {
        throw new Error(
          response.message || "Proposal was not returned by the API."
        );
      }
  ...
  /*
   * Change proposal status through the API.
   */
  const handleStatusChange = async (
    ...
    try {
      const response = await updateProposal(proposal.id, {
        status,
      });

      if (!response.data) {
        throw new Error(
          response.message || "Proposal update failed."
        );
      }
  ...
  /*
   * Duplicate using the backend.
   *
   * The real clientId is deliberately preserved.
   */
  const handleDuplicate = async (
    ...
    try {
      ...
      if (!response.data) {
        throw new Error(
          response.message || "Failed to duplicate proposal."
        );
      }
          ...
          <div className="mb-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
              ...
              <div className="flex gap-2 overflow-x-auto">
                {(["All", ...statusOptions] as const).map(
                  (item) => (
                    <button
                      key={item}
                      type="button"
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm">
            <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-500">
                This will permanently remove the proposal from your workspace.
